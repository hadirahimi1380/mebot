local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text ='گروه با موفقیت اضافه شد✅\n\n 》 @Tele_BanG'
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      local text = 'گروه با موفقیت حذف شد❎\n\n 🆔 @TelE_BanG'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function lock_group_links(msg, data, target)
  ...
  if group_lang then
   return reply_msg(msg.id,'🔒قفل لینڪ دَږ سۅپږگږۊه ازقبل فَعال بود🔒\n\n 🆔 @TelE_BanG', ok_cb, false)
   else
    return reply_msg(msg.id,'🔐Link Posting is already locked�\n\n 🆔 @TelE_BanG', ok_cb, false)
    end
    end
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'add' or matches[1] =='اد شو' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id,'🔃گروه از قبل اضافه شده است🔃 \n\n  🆔 @TelE_BanG', ok_cb, false)
			end
   ...
   if not is_super_group(msg) then
    return reply_msg(msg.id, '❌گروه اضاف نشده است❌\n\n 🆔 @TelE_BanG', ok_cb, false)
   end
